...
#define COMBA_2_BY_2_MUL(c0_, c1_, res_, A_, B_)          \
    asm volatile (                                        \
        /* Step 1: Multiply A[0] * B[0] */                \
        "movq 0(%[A]), %%rax            \n\t"             \
        "mulq 0(%[B])                   \n\t"             \
        "movq %%rax, 0(%[res])          \n\t"             \
        "movq %%rdx, %[c0]              \n\t"             \
                                                          \
        /* Step 2: Multiply A[0] * B[1] and A[1] * B[0] */\
        "xorq %[c1], %[c1]              \n\t"             \
        "movq 0(%[A]), %%rax            \n\t"             \
        "mulq 8(%[B])                   \n\t"             \
        "addq %%rax, %[c0]              \n\t"             \
...
#define REDUCE_4_LIMB_PRODUCT(k_, tmp1_, tmp2_, tmp3_, inv_, res_, mod_) \
    __asm__ volatile                               \
        ("///////////////////////////////////\n\t" \
         "movq   0(%[res]), %%rax            \n\t" \
         "mulq   %[modprime]                 \n\t" \
